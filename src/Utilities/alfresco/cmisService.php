...
use OlaHub\UserPortal\Libraries\cmisRepWrapper;
 ...
 * @since CMIS-1.0
 */
class cmisService extends cmisRepWrapper {
...
/* Utility functions */
	...
	function getMultiValuedProp($obj,$propName) {
		if (isset($obj->properties[$propName])) {
			return cmisRepWrapper::getAsArray($obj->properties[$propName]);
		}
		return array();
	}
	...
	function getTypeDescendants($typeId = null, $depth, $options = array ()) {
		...
		if ($typeId) {
			...
			$myURL = cmisRepWrapper :: getOpUrl($myURL, $hash_values);
		} else {
	...
	function getFolderParent($folderId, $options = array ()) { //yes
		...
		$myURL.= $this->GenURLQueryString($options);
		$ret = $this->doGet($myURL);
		$obj = cmisRepWrapper::extractObject($ret->body);
		$this->cacheObjectInfo($obj);
		return $obj;
	}
...
<?php
	...
	function query($q,$options=array()) {
		...
		$hash_values['q'] = $q;
		$post_value = cmisRepWrapper::processTemplate($query_template,$hash_values);
	...
	function getContentChanges($options = array()) {
		$myURL =  cmisRepWrapper :: processTemplate($this->workspace->links[LINK_CHANGES],$options);
...
<?php
	...
	function processPropertyTemplates($objectType, $propMap) {
		...
		foreach ($propMap as $propId => $propValue) {
			...
			$propertyContent .= cmisRepWrapper :: processTemplate($propTemplate, $hash_values);
		}
	...
	static function getContentEntry($content, $content_type = "application/octet-stream") {
		...
		if ($content) {
			return cmisRepWrapper :: processTemplate($contentTemplate, array (
...
<?php
    ...
    { // Yes
        ...
        $myURL = cmisRepWrapper :: getOpUrl($myURL, $options);
        ...
        $post_value = cmisRepWrapper :: processTemplate($entry_template, $hash_values);
	...
	function postEntry($url, $properties = array (), $content = null, $content_type = "application/octet-stream", $options = array ()) {
		...
		$myURL = cmisRepWrapper :: getOpUrl($url, $options);
		//DEBUG
	...
	function updateProperties($objectId, $properties = array (), $options = array ()) { // Yes
		...
		$obj_url = cmisRepWrapper :: getOpUrl($obj_url, $options);
		static $entry_template;
		if (!isset ($entry_template)) {
			$entry_template = CMISService :: getEntryTemplate();
		}
		...
		$put_value = cmisRepWrapper :: processTemplate($entry_template, $hash_values);
