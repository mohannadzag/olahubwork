...
class cmisRepWrapper
{
    ...
    function connect($url, $username, $password, $options)
    {
        ...
        if ($retval->code == HTTP_OK || $retval->code == HTTP_CREATED)
        {
            ...
            $this->workspace = cmisRepWrapper :: extractWorkspace($retval->body);
        }
    }
    ...
    function doRequest($url, $method = "GET", $content = null, $contentType = null, $charset = null)
    {
        ...
        {
            $url = cmisRepWrapper :: getOpUrl($url, $this->auth_options);
        }
    ...
    static function doXQuery($xmldata, $xquery)
    {
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: doXQueryFromNode($doc, $xquery);
    }
    ...
    static function doXQueryFromNode($xmlnode, $xquery)
    {
        ...
        foreach (cmisRepWrapper :: $namespaces as $nspre => $nsuri)
        {
	...
	static function extractAllowableActions($xmldata)
    {
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractAllowableActionsFromNode($doc);
    }
    ...
    static function extractObject($xmldata)
    {
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractObjectFromNode($doc);
    ...
    static function extractObjectFromNode($xmlnode)
    {
        ...
        $retval = new \stdClass();
        $retval->links = cmisRepWrapper :: getLinksArray($xmlnode);
        ...
        if (isset($children_feed_l) && is_object($children_feed_l) && is_object($children_feed_l->item(0))) {
	        ...
	        $retval->children = cmisRepWrapper :: extractObjectFeedFromNode($children_doc);
        }
		$retval->allowableActions = cmisRepWrapper :: extractAllowableActionsFromNode($xmlnode);
        return $retval;
    }
    ...
    static function extractTypeDef($xmldata)
    {
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractTypeDefFromNode($doc);
    ...
    static function extractTypeDefFromNode($xmlnode)
    {
        ...
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmisra:type/*");
        ...
        if (isset($childern_feed_l) && is_object($children_feed_l) && is_object($children_feed_l->item(0))) {
			...
			$xnode = $children_doc->importNode($children_feed,true); // Avoid Wrong Document Error
			$children_doc->appendChild($xnode);
	        $retval->children = cmisRepWrapper :: extractTypeFeedFromNode($children_doc);
        }
    ...
    static function extractObjectFeed($xmldata)
    {
        ...
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractObjectFeedFromNode($doc);
    }
    ...
    static function extractObjectFeedFromNode($xmlnode)
    {
        ...
        $numItemsNode = cmisRepWrapper::doXQueryFromNode($xmlnode, "/atom:feed/cmisra:numItems");
        ...
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "/atom:feed/atom:entry");
        foreach ($result as $node)
        {
            $obj = cmisRepWrapper :: extractObjectFromNode($node);
    ...
    static function extractTypeFeed($xmldata)
    {
        ...
        $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractTypeFeedFromNode($doc);
    }
    ...
    static function extractTypeFeedFromNode($xmlnode)
    {
        ...
        foreach ($result as $node)
        {
            $obj = cmisRepWrapper :: extractTypeDefFromNode($node);
            $retval->objectsById[$obj->id] = $obj;
            $retval->objectList[] = & $retval->objectsById[$obj->id];
        }
        return $retval;
    ...
    static function extractWorkspace($xmldata)
    {
        //Assumes only one workspace for now
            $doc = new \DOMDocument();
        $doc->loadXML($xmldata);
        return cmisRepWrapper :: extractWorkspaceFromNode($doc);
    }
    ...
    static function extractWorkspaceFromNode($xmlnode)
    {
        ...
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmisra:uritemplate");
        foreach ($result as $node)
        {
            $retval->uritemplates[$node->getElementsByTagName("type")->item(0)->nodeValue] = $node->getElementsByTagName("template")->item(0)->nodeValue;
        }
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//app:collection");
        foreach ($result as $node)
        {
            $retval->collections[$node->getElementsByTagName("collectionType")->item(0)->nodeValue] = $node->attributes->getNamedItem("href")->nodeValue;
        }
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmis:capabilities/*");
        foreach ($result as $node)
        {
            $retval->capabilities[$node->nodeName] = $node->nodeValue;
        }
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmisra:repositoryInfo/*[name()!='cmis:capabilities' and name()!='cmis:aclCapability']");
        foreach ($result as $node)
        {
            $retval->repositoryInfo[$node->nodeName] = $node->nodeValue;
        }
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmis:aclCapability/cmis:permissions");
        foreach ($result as $node)
        {
        ...
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmis:aclCapability/cmis:mapping");
        foreach ($result as $node)
        {
        ...
        $result = cmisRepWrapper :: doXQueryFromNode($xmlnode, "//cmis:aclCapability/*[name()!='cmis:permissions' and name()!='cmis:mapping']");
        foreach ($result as $node)
        {
            $retval->repositoryInfo[$node->nodeName] = $node->nodeValue;
        }
